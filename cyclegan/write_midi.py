...
def filter_short_notes(pm, delta=0.05):
    fpm = copy.deepcopy(pm)
    for instr in fpm.instruments:
        new_notes = []
        for note in instr.notes:
            if note.get_duration() >= delta:
...
#     # Create a PrettyMIDI object
#     midi = pretty_midi.PrettyMIDI(initial_tempo=tempo, resolution=beat_resolution)
...
def write_piano_rolls_to_midi(piano_rolls, program_nums=None, is_drum=None, filename='test.mid', velocity=100,
    ...
    for idx in range(len(piano_rolls)):
        ...
        # Add the instrument to the PrettyMIDI object
        midi.instruments.append(instrument)
    midi = filter_short_notes(midi, delta=0.1)
    # Write out the MIDI data
    midi.write(filename)
