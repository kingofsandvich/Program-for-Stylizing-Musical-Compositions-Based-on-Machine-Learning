...
def stylize():
    ...
    print(content["midi"])
    # print(binascii.unhexlify(content["midi"]))

    model, args = init_model(content["model"], content["AtoB"])

    # with open(r"C:\Users\1\Desktop\1_2.mid", "rb") as file:
    #     content["midi"] = binascii.hexlify(file.read())#.decode()
    # print(content["midi"])

    styled = run_model(model, content["midi"], args)

    styled = str(styled)[2:-1]
    # print("styled", type(styled), styled)
    res["midi"] = styled
    # except Exception as e:
    #     res = {"error": e.args}
