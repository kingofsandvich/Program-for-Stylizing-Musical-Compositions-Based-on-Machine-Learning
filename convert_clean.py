...
def get_midi_info(pm):
    ...
    midi_info = {
        ...
        'tempo': tempo, #tempi[0] if len(tc_times) == 1 else None
...
def get_merged(multitrack):
    ...
    for key in category_list:
        if category_list[key]:
            instr_tracks = list(multitrack[track_idx] for track_idx in category_list[key])
            mt = Multitrack(
                    name=multitrack.name,
                    resolution=multitrack.resolution,
                    tempo=None if multitrack.tempo is None else multitrack.tempo.copy(),
                    downbeat=None if multitrack.downbeat is None else multitrack.downbeat.copy(),
                    tracks=instr_tracks,
                )
            merged = mt.blend()
            tracks.append(Track(pianoroll=merged, program=program_dict[key], is_drum=key == 'Drums', name=key))
        else:
            tracks.append(Track(pianoroll=None, program=program_dict[key], is_drum=key == 'Drums', name=key))
    merged = Multitrack(tracks=tracks, tempo=multitrack.tempo, downbeat=multitrack.downbeat, resolution=multitrack.resolution, name=multitrack.name)
    return merged
...
def convert(filepath):
    ...
    return [merged, midi_name, midi_info]
